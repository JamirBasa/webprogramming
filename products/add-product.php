...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($image)) {
        ...
        $imageErr = 'File size is too big, make sure it is under 5MB.';
    }

    // If there are validation errors, return them as JSON
    if (!empty($codeErr) || !empty($nameErr) || !empty($categoryErr) || !empty($priceErr) || !empty($imageErr) || !empty($imageSizerErr)) {
        echo json_encode([
            'status' => 'error',
            'codeErr' => $codeErr,
            'nameErr' => $nameErr,
